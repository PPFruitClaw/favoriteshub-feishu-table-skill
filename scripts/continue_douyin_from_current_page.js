...
const MAX_STEPS = Number(process.argv[4] || 80);
const SAVE_EVERY = Number(process.argv[5] || 5);

if (!TARGET_ID) {
  console.error('usage: node continue_douyin_from_current_page.js <out> <targetId> [maxSteps] [saveEvery]');
  process.exit(1);
}
...
function save(outPath, records, meta = {}) {
  const obj = {
    ...
    mode: 'continue_from_current_page',
...
(async () => {
  ...
  if (fs.existsSync(OUT)) {
    try {
      ...
      existing = Array.isArray(old.records) ? old.records : [];
    } catch {}
  }
  const records = [...existing];
  const seen = new Set(records.map(r => r.link));
  let lastState = null;
  let stopReason = null;

  for (let step = 0; step <= MAX_STEPS; step += 1) {
    const state = oc(['browser', 'evaluate', '--target-id', TARGET_ID, '--fn', inspect, '--json']).result;
    lastState = state;
    if (!(state?.modal_id && state?.collect?.is_collected)) {
      stopReason = state?.collect?.state || 'not_collected_or_missing_modal';
      save(OUT, records, { stop_reason: stopReason, last_collected_modal_id: null, current_modal_id: state?.modal_id || null });
      process.stdout.write(JSON.stringify({ ok: true, total: records.length, stop_reason: stopReason, current_modal_id: state?.modal_id || null, out: OUT }, null, 2) + '\n');
      return;
    }
    ...
    if ((step + 1) % SAVE_EVERY === 0) {
      save(OUT, records, { stop_reason: null, last_collected_modal_id: state.modal_id, current_modal_id: state.modal_id });
    }
    oc(['browser', 'evaluate', '--target-id', TARGET_ID, '--fn', arrowDown, '--json']);
    await sleep(2200);
  }

  save(OUT, records, { stop_reason: 'max_steps_reached', last_collected_modal_id: lastState?.modal_id || null, current_modal_id: lastState?.modal_id || null });
  process.stdout.write(JSON.stringify({ ok: true, total: records.length, stop_reason: 'max_steps_reached', current_modal_id: lastState?.modal_id || null, out: OUT }, null, 2) + '\n');
})().catch(err => {
  try {
    const state = oc(['browser', 'evaluate', '--target-id', TARGET_ID, '--fn', inspect, '--json']).result;
    save(OUT, [], { stop_reason: 'exception', current_modal_id: state?.modal_id || null, error: String(err.message || err) });
  } catch {}
